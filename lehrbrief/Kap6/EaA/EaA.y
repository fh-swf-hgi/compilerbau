...
Expr: Expr PlusSymbol Expr  { $$ = newBinOpNode(AddNode, $1, $3); } 
	| Expr MinusSymbol Expr { $$ = newBinOpNode(SubNode, $1, $3); } 
	| Expr Multiplikation Expr { $$ = newBinOpNode(MulNode, $1, $3); } 
	| Expr Division Expr { $$ = newBinOpNode(DivNode, $1, $3); } 
	| Expr ModuloOperator Expr { $$ = newBinOpNode(ModNode, $1, $3); } 
	| Expr Exponentiation Expr { $$ = newBinOpNode(ExpoNode, $1, $3); } 
	| Sign { $$ = $1; } ;  
Sign 	: TypKonverter Atom { $$ = newUnOpNode(TypeNode, $2); } 
		| PlusSymbol Sign  { $$ = newUnOpNode(PosNode, $2); } 
		| MinusSymbol Sign { $$ = newUnOpNode(NegNode, $2); } 
		| Atom { $$ = $1; } ; 
Atom	: IntegerLiteral { $$ = newIntNode($1); } 
		| DoubleLiteral { $$ = newDoubleNode($1); } 
		| Bezeichner { $$ = newIdentNode($1); } 
